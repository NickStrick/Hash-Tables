...
def how_many_before_collisions(buckets, loops=1):
    """
    Roll random hash indexs into `buckets`
    and print how many rolls before a hash collision

    Runs loops number of times
    """
    results = []
    for i in range(loops):
        ...
        while True:
            ...
            if hash_index not in tried:
                ...
                break
        print(
            f"{buckets} buckets, {tries} tries, before collision. ({tries/buckets * 100:.1f}%)")
...
how_many_before_collisions(1024, 10)
how_many_before_collisions(4096, 10)
